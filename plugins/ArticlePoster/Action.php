...
class ArticlePoster_Action extends Typecho_Widget implements Widget_Interface_Do
{
    ...
    public function make(){
        ...
        $folder = dirname(__FILE__).'/poster/';
       	...
       	if(file_exists($folder.'Amore.ink_post_cid-'.$cid.'.png')){
        	$this->export(Helper::options()->pluginUrl.'/ArticlePoster/poster/Amore.ink_post_cid-'.$cid.'.png');
       	}
        ...
        $a = file_put_contents($folder.'Amore.ink_post_cid-'.$cid.'.png',base64_decode($res['img']));
        if($a){
        	$this->export(Helper::options()->pluginUrl.'/ArticlePoster/poster/Amore.ink_post_cid-'.$cid.'.png');
        }else{
